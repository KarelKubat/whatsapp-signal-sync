# Design Document: WhatsApp-Signal Sync CLI

This document outlines the design and architecture of the Go CLI application for syncing messages between WhatsApp and Signal.

## 1. System Overview

The program is a command-line interface (CLI) written in Go. Once running, it runs two background clients/services:
...
graph TD
    ...
    subgraph External
        ...
        SICLI[signal-cli Daemon Process]
    end
...
sequenceDiagram
    ...
    alt Account not linked
        CLI->>SI: Execute `signal-cli link -n whatsapp-sync`
...
- Uses `signal-cli`'s multi-device linking.
- Run `signal-cli --config <dir> link -n <device_name>`.
- Capture the output URI (`tsdevice:/?uuid=...`).
...
Once initialized:
1. **Signal Client**: Starts the subprocess `signal-cli --config <dir> -o json daemon` and redirects its stdout/stdin.
   - Runs a goroutine that parses line-by-line JSON-RPC outputs from stdout.
   - Handles `receive` notifications and maps them to events.
...
1. **Verify `signal-cli` connectivity**: Test run `signal-cli` locally and confirm standard linking commands.
2. **Create `go.mod` and directory structure**: Initialize the Go application.
3. **Implement Configuration and CLI Boilerplate**: Set up commands for `--setup` and daemon run.
4. **Implement WhatsApp Client**: Handle session storage, connection, and QR authentication.
5. **Implement Signal Daemon Wrapper**: Handle JSON-RPC subprocess piping, stdin/stdout parsing, group listing, and sending.
6. **Implement Sync Engine**: Wire the message event handlers and implement formatting/forwarding.
7. **Handle Media & Fallbacks**: Implement attachment download, upload, and path passing.
8. **End-to-End Testing**: Verify forwarding messages and media between channels.
